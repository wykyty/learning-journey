# Training a Sparse Autoencoder on T5 Decoder with TransformerLens
#
# This script trains a Sparse Autoencoder (SAE) on the decoder MLP activations of T5-large.
# Unlike the encoder-side SAE, the decoder receives cross-attention from the encoder and
# generates output tokens autoregressively.
#
# Key differences from the encoder-side SAE:
# - Decoder activations require both encoder input and decoder input
# - We use a summarization task (XSum) to provide meaningful encoder-decoder pairs
# - run_with_cache(text, decoder_input=token_ids) captures decoder-side activations
# - Target hook: decoder.12.hook_mlp_out
...
def train_sae(
    ...
    pbar = tqdm(range(config.total_steps), desc="Training SAE on Decoder")
...
@torch.no_grad()
def evaluate_batch_topk_sae(sae, collector, dataset_iter, model, n_batches=10):
    sae.eval()
    all_feature_acts, all_reconstructions, all_inputs = [], [], []
...
def plot_training_metrics(metrics):
    ...
    fig.suptitle("Decoder SAE Training Metrics", fontsize=14)
...
# ========== Main ==========
def main(sae_type: str = "standard"):
    """
    Args:
        sae_type: "standard" for ReLU + L1 SAE, "batch_topk" for BatchTopk SAE
    """
    # Load model
    ...
    # Initialize SAE based on type
    if sae_type == "batch_topk":
        config = BatchTopkSAEConfig()
        ...
        # Plot metrics (adapted for BatchTopk)
        fig, axes = plt.subplots(2, 3, figsize=(15, 8))
        ...
        # Evaluate
        eval_metrics = evaluate_batch_topk_sae(sae, collector, dataset_iter, model)
        print("\nEvaluation Metrics:")
        ...
        # Save
        save_path = Path("checkpoints/sae_t5_large_decoder_block12")
        sae.save_model(str(save_path))
        with open(save_path / "training_metrics.json", "w") as f:
            json.dump(metrics, f)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train SAE on T5 Decoder")
    parser.add_argument("--sae_type", type=str, default="standard",
                        choices=["standard", "batch_topk"],
                        help="Type of SAE to train: 'standard' or 'batch_topk'")
    args = parser.parse_args()
    main(sae_type=args.sae_type)
